...
public function ProcessPathIndicators(XComPathingPawn _PathingPawn) {
    ...
    if(class'GotchaAgainSettings'.default.bShowOverwatchTriggers || class'GotchaAgainSettings'.default.bShowActivationTriggers) {
        PrepareTriggerIndicators(PathingPawn.PathTiles,
                                 class'GotchaAgainSettings'.default.bShowOverwatchTriggers,
                                 class'GotchaAgainSettings'.default.bShowActivationTriggers,
                                 LoosingConcealAtIndex);
    }

    if(class'GotchaAgainSettings'.default.bShowPsiBombIndicator) PreparePsiBombIndicator(PathingPawn.PathTiles);
    if(class'GotchaAgainSettings'.default.bShowSmokeIndicator) PrepareSmokeIndicator(PathingPawn.PathTiles);
...
public function OnUpdateWaypointMode() {
    if(!class'GotchaAgainSettings'.default.bUseCustomPathIndicatorSystem) return;
...
private function int PrepareDefaultIndicators(const out array<TTile> PathTiles, int LoosingConcealAtIndex) {
    ...
    for(i = 0; i < PathTiles.Length; i++) {
        ...
        // Noise-markers
        if(class'GotchaAgainSettings'.default.bShowNoiseIndicators || !BrokeConceal) {
    ...
    if(class'GotchaAgainSettings'.default.bShowNoiseIndicators) {
    ...
    for(i = 0; i < PathIndicatorLocations.Length; i++) {
        for(j = PathIndicatorLocations[i].IndicatorTypes.Length - 1; j > -1; j--) {
            if(!BrokeConceal) {
                ...
                } else if(PathIndicatorLocations[i].IndicatorTypes[j] == eMakeNoise && !class'GotchaAgainSettings'.default.bShowNoiseIndicators) {
    ...
    foreach RevealedEnemies(EnemyUnit) {
        ...
        if(UnitLOSValues.bClearLOS) {
            if(UnitLOSValues.bWithinRegularRange) {
                ...
                } else if(class'GotchaAgainSettings'.default.bShowOverwatchTriggerForSuppression && EnemyUnit.ReserveActionPoints.Find('Suppression') != INDEX_NONE) {
